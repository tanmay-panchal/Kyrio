    ...
    data: [
        {
            ...
            methodClick: function () {
                $.ModalGiftVoucher(function () {
                    ...
                    if (expiry == "months_1") {
                        VoucherExpiryDate = moment().add(1, 'month').format("YYYY/MM/DD");
                    }
                    else if (expiry == "months_2") {
                        VoucherExpiryDate = moment().add(2, 'month').format("YYYY/MM/DD");
                    }
                    else if (expiry == "months_3") {
                        VoucherExpiryDate = moment().add(3, 'month').format("YYYY/MM/DD");
                    }
                    else if (expiry == "months_6") {
                        VoucherExpiryDate = moment().add(6, 'month').format("YYYY/MM/DD");
                    }
                    else if (expiry == "years_1") {
                        VoucherExpiryDate = moment().add(1, 'years').format("YYYY/MM/DD");
                    }
                    else if (expiry == "end_of_month") {
                        VoucherExpiryDate = moment().endOf('month').format("YYYY/MM/DD");
                    }
...
var AddItem = function (item, locationid) {
    ...
    itemElement.find("#PriceDisplay").text(Window.CurrencySymbol + $.number(Price, Window.NumberDecimal, '.', ','));
    itemElement.find("#DescriptionDisplay").text(Description);
    itemElement.find("[name='priceItem']").val(Price);
    ...
    $(".containt-serivce [name='staffItem']:last").SetValueSelect2ID(Window.UserID);
    ...
    $(".containt-serivce #deleteManualDiscountItem").click(function () {
        ...
        itemElement.find("[name='priceItem']").val(itemElement.attr("OrgPrice"));
        CalculateItem(itemElement);
        SumTotal();
    })
    $(".containt-serivce .delete-service").click(function () {
        var count = 0;
        var that = $(this);
        $("#divPayment .payment").each(function () {
            count++;
        })
        if (count > 0) {
            PNotify.notice({
...
var SumTotal = function () {
    ...
    var countitem = 0;
    var TaxDetail = [];
    $(".order-detail #containtItem .containt-serivce").each(function () {
        countitem++;
    ...
    $("#SubTotalBeForeTax").text(Window.CurrencySymbol + $.number(SubTotalBeForeTax, Window.NumberDecimal, '.', ','));
    //line tax detail
    $("#divTaxDetail").html("");
    ...
    $("#Total").text(Window.CurrencySymbol + $.number(Total, Window.NumberDecimal, '.', ','));
    ...
    $("#Balance").text(Window.CurrencySymbol + $.number(Balance, Window.NumberDecimal, '.', ','));
    $("#Pay").val(parseFloat(Balance).toFixed(Window.NumberDecimal));
    //an hien khung 
    if (countitem == 0) {
        $("#contentLeft .no-order").show();
        $("#contentLeft .order-detail").hide();
    }
    else {
        $("#contentLeft .no-order").hide();
        $("#contentLeft .order-detail").show();
    }
    if ($("#IsRefund").val() == "") {
        if (Balance > 0) {
            $("#contentRight #divPay").show();
            $("#contentRight #divFullPay").hide();
            //show save part paid
            if (TotalPay > 0) {
            ...
            $("#contentRight #divPay").hide();
            if (TotalPay > 0) {
                $("#contentRight #divFullPay").show();
            }
        }
    }
    else {
        ...
        if (data.Result.length > 0) {
            ...
            $.each(data.Result, function () {
                if (i % 2 != 0) {
                    ...
                    $("#divPaymentMethod").append(line[0].outerHTML);
                }
...
var AddItemPayment = function (PaymentTypeID, PaymentAmount, PaymentTypeName, VoucherID, VoucherCode, PayForServiceID) {
    ...
    if (findsame == false) {
        ...
        if (count == 0) {
            $("#divPayment").append("<hr/>");
        }
        $("#divPayment").append(element[0].outerHTML);
        $(".payment .delete-payment").click(function () {
...
var SaveInvoice = function (InvoiceStatus) {
    ...
    entity["InvoiceDate"] = moment().format("YYYY/MM/DD HH:mm");
    entity["DueDate"] = moment().format("YYYY/MM/DD");
    entity["InvoiceStatus"] = InvoiceStatus;
    entity["LocationID"] = localStorage.getItem("locationidscheul");
...
var ShowClientInfo = function (renposive) {
    ...
    containtSearch.hide();
    containtSearch.closest("div.form-group").prepend(clientTitle);

    var closeClientDetail = $('<div class="form-group modal-backdrop d-flex fade show flex-column position-absolute" style="z-index: 3000">'
...
var ShowClientDetail = function (ClientID) {
    ...
    $.RequestAjax(urlGetClientBaseIdForAppointment, JSON.stringify({
        ...
        $.RequestAjaxText("/ContentHtml/Client/Desktop/Detail/ClientDetail.html", function (html) {
            Body = $(html.replace("@Represent", ($.trim(data.Client.FirstName) == "" ? '' : data.Client.FirstName.toString().charAt(0)))
...
$(function () {
    ...
    $.ModalGiftVoucher = function (clickSave) {
        ...
        $("#ExpiryManual").data('daterangepicker').setStartDate(moment()._d);
        $("#ExpiryManual").data('daterangepicker').setEndDate(moment()._d);
        $("#ExpiryManual").val(moment().format("dddd DD MMM, YYYY"));
        ...
        if (code == "") {
            ...
            $.RequestAjax(urlSearchVoucherCode, JSON.stringify({
                ...
                if (data.Results.length > 0) {
                    ...
                    //check ExpireDate
                    if (Voucher.ExpireDate != null) {
                        if (moment(Voucher.ExpireDate).isBefore(moment())) {
